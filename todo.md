...
- [ ] P1 编写 `scripts/check_env.py` 环境检查脚本
- [ ] P1 统一 `Makefile` / `taskipy` 快捷命令（lint / test / build / start）
...
- [x] P0 Schema 校验与对齐
- [x] P0 目标列选择接口
- [ ] P1 **数据库连接上传**：MySQL / PostgreSQL / ClickHouse / SQLite（通过 SQLAlchemy + 自定义 SQL）
- [ ] P1 数据集预览接口（前 N 行 + 统计摘要）
- [ ] P1 数据质量六维报告（完整性、一致性、准确性、时效性、唯一性、有效性）
- [ ] P1 数据清洗规则可配置（业务规则提取入口）
...
- [ ] P1 **Prefect Artifact**：在 UI 中直接查看 leaderboard、特征重要性、数据质量报告
- [ ] P1 **Prefect State/Cache**：对数据加载、Embedding 生成等耗时步骤启用结果缓存
...
- [ ] P1 **模型搜索空间动态裁剪**：小数据加入 KNN/SVM/NN，大数据仅 LightGBM/XGBoost/线性模型
...
- [ ] P1 **缺失值策略按缺失率分级**：<5% 删行、5-30% 填充、>50% 丢弃或缺失指示列
- [ ] P1 **异常值处理**：Isolation Forest 标记 + Schema 约束截断
- [ ] P1 **数值缩放策略**：根据分布自动选 RobustScaler / StandardScaler / MinMaxScaler
- [ ] P1 **类别编码按基数选择**：低基数 One-Hot、高基数 Target Encoding（必须 CV 内 fit）
- [ ] P1 **高基数低频项合并**为 "__other__"
- [ ] P1 **文本 Embedding**：sentence-transformers 本地提取（失败则跳过）
- [ ] P2 **交叉特征**：基于树模型重要性反馈自动尝试二阶交叉
- [ ] P2 **特征选择**：Filter / L1 / Permutation Importance / SHAP
- [ ] P2 **时间序列特征**：lag、rolling window、sin/cos 周期编码
...
- [ ] P1 **条件过采样**：SMOTE（纯数值） / SMOTENC（混合类型）
- [ ] P1 **条件欠采样**：Random Under-sampling（大数据场景）
- [ ] P1 **组合采样**：SMOTEENN / SMOTETomek
- [ ] P1 **采样必须在训练集划分后执行**，验证集保持原始分布
- [ ] P1 **阈值调优**：根据业务错误成本自动调整分类阈值
- [ ] P2 **条件降维**：
  - 高相关性剔除（|r| > 0.95）
  - 低方差剔除
  - PCA（保留 95% 方差，仅数值特征）
...
- [ ] P1 **多种 CV 策略自动选择**：
  - 标准表格：StratifiedKFold / KFold
  - 时间序列：TimeSeriesSplit
  - 分组数据：GroupKFold
- [ ] P1 **CV 内完成所有预处理**，确保无数据泄露
- [ ] P1 **嵌套 CV**：内层调参、外层评估，避免 optimistic bias
- [ ] P1 **主指标自动选择**：不平衡 → AUC-PR / MCC；平衡 → F1；回归 → RMSE / MAE
- [ ] P1 **自动集成验证**：检查集成是否带来 ≥2% 提升，否则回退单最优模型
...
- [x] P0 SHAP 汇总图
- [ ] P1 **TreeSHAP 全局 + 单样本解释**（force / waterfall 图）
- [ ] P1 **Permutation Importance**（用于泄露检测和无偏重要性）
- [ ] P1 数据质量摘要嵌入报告
- [ ] P1 混淆矩阵、ROC/PR 曲线、回归残差图
- [ ] P2 **反事实解释**（针对拒绝/负面决策）
...
- [ ] P1 **批量预测**：上传 CSV 返回 predictions.csv
- [ ] P1 **配置快照（config_snapshot）**：保存数据 hash、Schema、超参、模型版本，保证可复现
