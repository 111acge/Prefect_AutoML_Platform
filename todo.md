...
1. **P1 端到端回归测试**：跑通 iris 完整训练，验证报告、预测、模型下载均正常。
2. **P2 前端报告业务解读展示**：任务详情页可直接查看业务解读摘要（当前仅在 HTML 报告中）。
3. **P2 断点续跑**：基于 Prefect Task 状态实现崩溃后恢复。
4. **P2 Target Encoding 折内 fit**：进一步降低高基数类别编码的泄露风险。
