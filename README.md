...
端到端全自动机器学习平台，基于 **Prefect** 工作流编排 + **AutoGluon** 自动建模，支持从数据清洗到模型部署的全流程自动化。后端同时提供自然语言意图解析 API，可作为工作流组件集成。
...
Prefect AutoML Platform 是一个面向结构化数据的低代码 AutoML 平台。用户可以通过 Web 界面上传数据集、选择目标列与任务类型，平台自动完成：
...
- [x] 国际化（i18n）支持：已支持中文与英文切换
- [ ] 修复文件上传路径遍历，并增加扩展名与大小校验
- [ ] 升级 starlette / fastapi 与 pydantic-settings 修复已知 CVE
- [ ] LLM 错误信息脱敏，避免第三方返回中泄露 API Key 片段
- [ ] 等待上游依赖兼容后适配 Python 3.15

---

## 安全声明

本项目定位为**本地或可信内网自用工具**，不提供公共服务，也不建议直接部署到公网。

当前版本存在以下与部署方式相关的限制，请自行评估风险：

- **无认证/鉴权**：所有 API 默认对外开放，任何可访问后端的用户都能调用训练、预测、下载模型等接口。
- **CORS 宽松配置**：开发模式下 `allow_origins=["*"]` 与 `allow_credentials=True` 同时开启，便于本地前后端联调；公网部署需自行收紧。
- **API 返回绝对路径**：数据集与运行记录的接口响应中包含服务器本地绝对路径（`file_path`、`output_dir`）。
- **外部数据库连接**：数据库连接功能允许用户指定任意 host 与 SQLite 文件路径，并执行自定义 SQL；请仅连接自己信任的数据源。
- **异常信息直接返回**：部分接口在出错时会将原始异常信息返回给前端，便于本地调试。
- **依赖漏洞**：部分依赖（如 starlette、pydantic-settings）存在已知 CVE，详见 [TODO / 路线图](#todo--路线图)。

在公网或多人共享环境中使用时，请自行增加反向代理、身份认证、CORS 限制与请求限流。

---

## AI 生成内容声明

本项目部分代码由生成式人工智能（Kimi，月之暗面科技有限公司）辅助生成，并经人工审查、修改和测试。原始生成内容受相关服务条款约束，最终项目的知识产权归属及责任由项目维护者承担。
